...
async def analyze_text(payload: dict):
    ...
    sensitivity = payload.get("sensitivity", 80)
    result = analyze_document(text=text, serpapi_key=serpapi_key, sensitivity=sensitivity)
    return {"result": result}

@app.post("/analyze/pdf")
async def analyze_pdf_endpoint(file: UploadFile = File(...), serpapi_key: str = None, sensitivity: int = 80):
    pdf_bytes = await file.read()
    result = analyze_pdf(pdf_bytes=pdf_bytes, serpapi_key=serpapi_key, sensitivity=sensitivity)
    return {"result": result}
